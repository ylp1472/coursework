...
import OrderEditModal from '../../components/admin/ordersdashboard/OrderEditModal';
						...
						<div className="w-full h-full flex flex-col gap-4 container mx-auto p-4">
							<div className="w-full flex flex-col gap-4 container mx-auto p-4 text-black bg-white rounded">
								<span className="w-full flex flex-row justify-between items-center">
									...
									<button onClick={fetchOrders} className="flex flex-row items-center justify-center gap-2 text-xs text-red-500 px-2 py-2 rounded outline-none bg-white hover:bg-gray-100 focus:bg-gray-100">
								...
								<table className="w-full table-auto">
									<thead className="sticky top-0 bg-white">
										<tr>
											...
											<th className="text-red-500 bg-red-100 text-sm px-4 py-4 min-w-[250px]">Cart</th>
											<th className="text-red-500 bg-red-100 text-sm px-4 py-4 min-w-[150px]">Total Value</th>
											<th className="text-red-500 bg-red-100 text-sm px-4 py-4 min-w-[150px]">Status</th>
											<th className="text-red-500 bg-red-100 text-sm px-4 py-4 min-w-[100px]">Actions</th>
										</tr>
									</thead>
									...
									<tbody>
										{orders.map(order => (
											<tr key={order._id}>
												<td className="border-t-2 text-sm text-center px-4 py-4">{order._id}</td>
												<td className="border-t-2 text-sm text-center px-4 py-4">{order.email}</td>
												<td className="border-t-2 text-sm px-4 py-4">
													<ul>
														{order.cart.map((item) => (
															<li key={item._id} className="py-1">{item.quantity} x <a className="font-medium underline" href={`/products/${item.productID}`} target="_blank" rel="noreferrer">{item.name}</a><br />= LKR {item.price}</li>
														))}
													</ul>
												</td>
												<td className="border-t-2 text-sm text-center px-4 py-4">LKR {order.totalValue}</td>
